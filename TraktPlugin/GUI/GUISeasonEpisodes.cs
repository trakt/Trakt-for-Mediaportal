...
namespace TraktPlugin.GUI
{
    public class GUISeasonEpisodes : GUIWindow
    {
        ...
        private void AddEpisodeToLibrary(TraktEpisode episode)
        {
            Thread syncThread = new Thread(delegate(object obj)
            {
                TraktAPI.TraktAPI.SyncEpisodeLibrary(CreateEpisodeSyncData(obj as TraktEpisode), TraktSyncModes.library);
            })
            {
                IsBackground = true,
        ...
        private void MarkEpisodeAsUnWatched(TraktEpisode episode)
        {
            Thread syncThread = new Thread(delegate(object obj)
            {
                TraktAPI.TraktAPI.SyncEpisodeLibrary(CreateEpisodeSyncData(obj as TraktEpisode), TraktSyncModes.unseen);
            })
            {
                IsBackground = true,
    ...
    public class GUITraktSeasonEpisodeListItem : GUIListItem
    {
        ...
        {
            GUISeasonEpisodes window = GUIWindowManager.GetWindow(GUIWindowManager.ActiveWindow) as GUISeasonEpisodes;
            if (window != null)
            {
                GUIListItem selectedItem = GUIControl.GetSelectedListItem((int)TraktGUIWindows.SeasonEpisodes, (int)TraktGUIControls.Facade);
